...
const IntelligenceMatrix: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const constNodes = TECH_NODES.map((tag, i) => ({
            ...
            r: hex2rgb(PALETTE[i % PALETTE.length]),
        ...
        // ── 5. DATA BEAMS ──────────────────────────────────────────────────
        interface Beam { fx:number;fy:number;tx:number;ty:number;t:number;speed:number;color:string;trail:{x:number;y:number}[] }
        const beams: Beam[] = [];
        let beamTimer = 0;
        const spawnBeam = () => {
            const from = orbs[Math.floor(Math.random() * orbs.length)];
            const to   = constNodes[Math.floor(Math.random() * constNodes.length)];
            beams.push({ fx: from.x, fy: from.y, tx: to.x, ty: to.y, t: 0, speed: 0.009 + Math.random() * 0.009, color: from.color, trail: [] });
        };
        ...
        // ── 7. SHOOTING COMETS ────────────────────────────────────────────
        interface Comet { x:number;y:number;vx:number;vy:number;len:number;color:string;life:number;maxLife:number }
        const comets: Comet[] = [];
        let cometTimer = 0;
        const spawnComet = () => {
        ...
        const draw = () => {
            ...
            for (let i = 0; i < constNodes.length; i++) {
                for (let j = i + 1; j < constNodes.length; j++) {
                    ...
                    if (d > 190) continue;
                    const alpha = (1 - d / 190) * 0.11;
                    ctx.strokeStyle = `rgba(${a.r.r},${a.r.g},${a.r.b},${alpha})`;
                    ctx.lineWidth = 0.6;
            ...
            for (let i = 0; i < constNodes.length; i += 4) {
                ...
                const d = Math.hypot(a.x - b.x, a.y - b.y);
                if (d < 100) continue;
                const alpha = 0.06 + Math.sin(frame * 0.02 + i) * 0.03;
            ...
            constNodes.forEach(n => {
                ...
                g.addColorStop(0, n.color + Math.round(a * 0.4 * 255).toString(16).padStart(2,'0'));
                g.addColorStop(1, 'transparent');
                ctx.fillStyle = g;
            ...
            // ─ Orbital paths ───────────────────────────────────────────────
            orbs.slice(1).forEach(o => {
                ctx.save();
                ctx.strokeStyle = o.color + '25';
                ctx.lineWidth = 0.8;
                ctx.setLineDash([5, 9]);
            ...
            orbs.forEach((o, i) => {
                ...
                aura.addColorStop(0, o.color + Math.round((0.25 + pulse * 0.25) * 255).toString(16).padStart(2,'0'));
                aura.addColorStop(0.4, o.color + '18');
                ...
                if (i === 0 && (o as any).rings) {
                    for (let r = 1; r <= 3; r++) {
                        const rAlpha = (0.18 - r * 0.04 + pulse * 0.08);
                        ctx.beginPath();
                        ctx.arc(o.x, o.y, o.size * 1.8 + r * 14 + pulse * 4, 0, Math.PI * 2);
                        ctx.strokeStyle = o.color + Math.round(Math.max(0,rAlpha) * 255).toString(16).padStart(2,'0');
                ...
                cg.addColorStop(0.35, o.color);
                cg.addColorStop(1, o.color + 'aa');
                ctx.fillStyle = cg;
            ...
            const llm = orbs[0];
            const flareAlpha = 0.12 + Math.sin(frame * 0.04) * 0.06;
            ...
            [0, 45, 90, 135, 180, 225, 270, 315].forEach(deg => {
                ...
                sg.addColorStop(0, `rgba(0,247,255,${flareAlpha})`);
            ...
            dust.forEach(d => {
                ...
                ctx.fillStyle = `rgba(${rgb.r},${rgb.g},${rgb.b},${d.a})`;
                ctx.fill();
            });
